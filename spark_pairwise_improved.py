...
def partition_gen(iterator):
    ...
    for term, docs in iterator:
        # directly reuse gen_pairs_from_list for correctness
...
if __name__ == "__main__":
    ...
    # combineByKey = map-side aggregation for scalability
    ...
    ).cache()      # cached because reused exactly once
    ...
    jaccard_rdd = reduced.map(
        lambda kv: compute_jaccard(kv[0], kv[1])
