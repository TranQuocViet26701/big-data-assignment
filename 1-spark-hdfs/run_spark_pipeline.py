...
class SparkPipelineRunner:
    ...
    def ranking_search(self):
        """Execute Stage 3: Ranking Search based on Similarity with Spark"""
        print("\n" + "="*80)
        print("STAGE 3: Ranking Search based on Similarity with Spark")
        print("="*80)

        ranking_search_script = os.path.join(self.script_dir, 'ranking_search.py')

        # Use the actual output path (with part-* files)
        index_files = f"{self.stage2_output}/part-*"
        try:
            # Construct spark-submit command
            cmd = f"""spark-submit \\
                --master yarn \\
                ...
                --driver-memory {self.args.driver_memory} \\
                --conf spark.dynamicAllocation.enabled=false \\
                {ranking_search_script} \\
                {index_files} \\
                {self.ranking_search_output} \\
                {self.mode} """

            start_time = time.time()
            result = self.run_command(cmd, "Running Spark Ranking Search similarity")
            stage3_time = time.time() - start_time

            if result is None:
                raise Exception("Stage 3 (Search) failed")
