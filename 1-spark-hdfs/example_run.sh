#!/bin/bash
#
# Example script demonstrating how to run the Spark pipeline
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Spark Pipeline Example Execution${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
run_pipeline() {
    ...
    python3 "${SCRIPT_DIR}/run_spark_pipeline.py" \
        --num-books "$num_books" \
...
case "$TEST_SIZE" in
    quick)
        ...
        run_pipeline 10 "adventure travel exploration journey" 2 "2G"
        ;;
    ...
    medium)
        ...
        run_pipeline 100 "wildlife conservation hunting animals nature" 4 "4G"

        echo -e "${BLUE}Test 2: Science and technology${NC}"
        run_pipeline 100 "science technology innovation research discovery" 4 "4G"
        ;;
    ...
    full)
        ...
        if hdfs dfs -test -e /gutenberg-input-50 2>/dev/null; then
            ...
            run_pipeline 50 "literature poetry prose writing author" 3 "3G"
        else
        ...
        if hdfs dfs -test -e /gutenberg-input-100 2>/dev/null; then
            ...
            run_pipeline 100 "history war battle victory defeat empire" 4 "4G"
            ...
            run_pipeline_with_file 100 "$QUERY_FILE" 4 "4G"
            rm -f "$QUERY_FILE"
        else
        ...
        if hdfs dfs -test -e /gutenberg-input-200 2>/dev/null; then
            ...
            run_pipeline 200 "philosophy truth knowledge wisdom reason thought" 6 "6G"
        else
...
echo ""
echo "View results:"
echo "  cat ${SCRIPT_DIR}/spark_metrics.csv"
echo ""
echo "View latest similarity results:"
echo "  hdfs dfs -ls /spark-jpii-* | tail -1"
echo "  hdfs dfs -cat /spark-jpii-*/part-* | head -20"
echo ""
