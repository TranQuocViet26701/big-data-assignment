#!/bin/bash
################################################################################
# Spark Pipeline Benchmark Script
#
# Automatically runs the Spark pipeline in both JPII and Pairwise modes
# across multiple dataset sizes for performance comparison and metrics collection.
#
# Usage:
#   ./benchmark_spark_modes.sh [OPTIONS]
#
# Options:
...
#   ./benchmark_spark_modes.sh
#
#   # Run only JPII mode
#   ./benchmark_spark_modes.sh --mode jpii
#
#   # Run only pairwise on small datasets
#   ./benchmark_spark_modes.sh --mode pairwise --datasets "10 50"
#
#   # Custom query file and configuration
#   ./benchmark_spark_modes.sh --query-file custom_query.txt --executors 8
################################################################################
...
if [[ "$MODE" == "jpii" || "$MODE" == "both" ]]; then
    ...
    for NUM_BOOKS in $DATASETS; do
        ...
        if python3 "$SCRIPT_DIR/run_spark_pipeline.py" \
            --mode jpii \
            --num-books "$NUM_BOOKS" \
...
if [[ "$MODE" == "jpii" || "$MODE" == "both" ]]; then
    echo "JPII Metrics:      $SCRIPT_DIR/spark_jpii_metrics.csv"
fi

if [[ "$MODE" == "pairwise" || "$MODE" == "both" ]]; then
    echo "Pairwise Metrics:  $SCRIPT_DIR/spark_pairwise_metrics.csv"
fi
...
if [[ "$MODE" == "jpii" || "$MODE" == "both" ]] && [[ -f "$SCRIPT_DIR/spark_jpii_metrics.csv" ]]; then
    echo ""
    print_info "JPII Results (last $SUCCESSFUL_RUNS runs):"
    echo ""
    echo "Books | Stage1(s) | Stage2(s) | Total(s) | Pairs | Throughput"
    echo "------|-----------|-----------|----------|-------|------------"
    tail -n "$SUCCESSFUL_RUNS" "$SCRIPT_DIR/spark_jpii_metrics.csv" | \
        awk -F',' 'NR>1 {printf "%5s | %9.2f | %9.2f | %8.2f | %5s | %10.4f\n", $3, $4, $5, $6, $10, $11}'
fi

if [[ "$MODE" == "pairwise" || "$MODE" == "both" ]] && [[ -f "$SCRIPT_DIR/spark_pairwise_metrics.csv" ]]; then
    echo ""
    print_info "Pairwise Results (last $SUCCESSFUL_RUNS runs):"
    echo ""
    echo "Books | Stage1(s) | Stage2(s) | Total(s) | Pairs | Throughput(pairs/s)"
    echo "------|-----------|-----------|----------|-------|-------------------"
    tail -n "$SUCCESSFUL_RUNS" "$SCRIPT_DIR/spark_pairwise_metrics.csv" | \
